...
function decryptSecretKey(instanceCredentials, accountCredentials) {
    // XXX don't forget to use u.encryptionKeyVersion if present
    const privateKey = forge.pki.privateKeyFromPem(
        instanceCredentials.privateKey);
    const encryptedSecretKey = forge.util.decode64(
        accountCredentials.secretKey);
    ...
    return getStoredCredentials(instanceId, log, (err, creds) => {
        ...
        if (newConf.users) {
            newConf.users.forEach(u => {
                const secretKey = decryptSecretKey(creds, u);
