...
const managementEndpoint = process.env.MANAGEMENT_ENDPOINT ||
    'http://pensieve-api-env-dev.vbdktucxuq.us-west-2.elasticbeanstalk.com/api/v1/instance' ||
...
function patchConfiguration(instanceId, newConf, log, cb) {
    ...
    return getStoredCredentials(instanceId, log, (err, creds) => {
        ...
        if (newConf.locations) {
            Object.values(newConf.locations).forEach(l => {
                const location = {};
                ...
                location.legacyAwsBehavior = l.legacyAwsBehavior;
                locations[l.name] = location;
            });
            console.log('CONSTRAINTS PARSED', JSON.stringify(locations, null, 4));
