...
const multipleBackendGateway = {
    ...
    getDiskUsage: (reqUids, callback) => {
        const fileLocations = Object.keys(config.locationConstraints).
            filter(l => config.locationConstraints[l].type === 'file');
        ...
        if (fileLocations.length === 0) {
            return process.nextTick(callback, null, zero);
        }

        clients[fileLocations[0]].getDiskUsage(reqUids, callback);
    },
};
